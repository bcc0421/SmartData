...
class Channel(object):
    ...
    def __init__(self, apiKey, secretKey, arr_curlOpts=None):
        ...
        if (not isinstance(self._curlOpts, dict)):
            raise ChannelException('invalid param -arr_curlopt is not an dict', Channel.CHANNEL_SDK_INIT_FAIL)
        ...
        try:
            ...
            if (push_type == Channel.PUSH_TO_USER):
                if (not arrArgs.has_key(Channel.USER_ID) or
                    ...
                    raise ChannelException("userId should be specified in optional when pushType is PUSH_TO_USER",
                ...
                if (not arrArgs.has_key(Channel.TAG_NAME) or
                    ...
                    raise ChannelException("tagName should be specified in optional when pushType is PUSH_TO_TAG",
                                           self.CHANNEL_SDK_PARAM)
    ...
    def _baseControl(self, opt):
        ...
        request = requests.post(url, data=opt, headers=headers, verify=False)
        return request
